...
pthread_cond_t condImageHeap[MAX_CAMERA_NUM];
...
void clearSystemQueueMsg(void)
{
	...
	for(i = 0; i < MAX_CAMERA_NUM; i ++)
	{
		do
		{
			ret = xQueueReceive((key_t)KEY_SYNC_CAM_TIME_STAMP_MSG + i,(void **)&pointer,0);
		}
		while(ret != -1);
	}
	...
	for(i = 0; i < MAX_CAMERA_NUM; i ++)
	{
		do
		{
			ret = xQueueReceive((key_t)KEY_CAMERA_RESET_MSG + i,(void **)&pointer,0);
		}
		while(ret != -1);
	}
	...
	for(i = 0; i < MAX_CAMERA_NUM; i ++)
	{
		do
		{
			ret = xQueueReceive((key_t)KEY_IMAGE_HANDLER_MSG + i,(void **)&pointer,0);
		}
		while(ret != -1);
	}
	...
	if(imageHeap[index].heap != NULL && imageHeap[index].depth != 0)
    {
        for(i = 0; i < imageHeap[index].depth; i ++)
        {
            if(imageHeap[index].heap[i] != NULL)
            {
                ...
                {
                    if(imageHeap[index].heap[i]->image->image != NULL)
                    {
                        free(imageHeap[index].heap[i]->image->image);
                        imageHeap[index].heap[i]->image->image = NULL;
                    }
				...
				if(imageHeap[index].heap[i]->time_stamp != NULL)
                {
                    free(imageHeap[index].heap[i]->time_stamp);
                    imageHeap[index].heap[i]->time_stamp = NULL;
                }
...
int allocateImageHeap(unsigned char index,unsigned short depth,unsigned int image_size)
{
	...
	imageHeap[index].heap = calloc(imageHeap[index].depth, sizeof(struct ImageHeapUnit));
	...
	{	
		...
		imageHeap[index].heap[i]->image = NULL;
		imageHeap[index].heap[i]->image = (struct ImageBuffer *)malloc(sizeof(struct ImageBuffer));
		if(imageHeap[index].heap[i]->image == NULL)
		{
		...
		memset(imageHeap[index].heap[i]->image,0,sizeof(struct ImageBuffer));

		imageHeap[index].heap[i]->image->size = image_size;

		imageHeap[index].heap[i]->image->image = (char *)malloc(image_size * sizeof(char));
		if(imageHeap[index].heap[i]->image->image == NULL)
		{
			fprintf(stderr, "%s: malloc imageHeap[%d].heap[%d]->image->image failed\n",__func__,i,index);
			return -1;
		}

		memset(imageHeap[index].heap[i]->image->image,0,image_size);

		imageHeap[index].heap[i]->time_stamp = NULL;
		if(imageHeap[index].heap[i]->time_stamp != NULL)
		{
			fprintf(stderr, "%s: imageHeap[%d].heap[%d]->time_stamp does not null\n",__func__,i,index);
			return -1;
		}

		imageHeap[index].heap[i]->time_stamp = NULL;
		imageHeap[index].heap[i]->time_stamp = (struct SyncCamTimeStamp *)malloc(sizeof(struct SyncCamTimeStamp));
		if(imageHeap[index].heap[i]->time_stamp == NULL)
		{
			fprintf(stderr, "%s: malloc imageHeap[%d].heap[%d]->time_stamp failed\n",__func__,i,index);
			return -1;
		}

		memset(imageHeap[index].heap[i]->time_stamp,0,sizeof(struct SyncCamTimeStamp));
	}
...
int imageHeapGet(unsigned char index, struct ImageHeapUnit *data)
{
	if(data == NULL)
	{
	...
	if(imageHeap[index].cnt > 0)
	{
		memcpy(data,imageHeap[index].heap[imageHeap[index].get_ptr],sizeof(struct ImageHeapUnit));
...
static void syncAndMutexCreate(void)
{
	...
	for(i = 0; i < MAX_CAMERA_NUM; i ++)
	{
		...
		pthread_cond_init(&condImageHeap[i], NULL);
	}
...
static int pthreadCreate(void *args)
{
    ...
    if(cmdArgs.camera_module == 0)
    {
		...
		{
			ret = pthread_create(&tid_ui3240[cmdArgs.camera_index],NULL,thread_ui3240,&cmdArgs);
			if(0 != ret)
			{
				fprintf(stderr, "%s: create thread_ui3240[%d] failed\n",__func__,cmdArgs.camera_index);
			}
    ...
    else
    {
		for(cmdArgs.camera_index = 0; cmdArgs.camera_index < cmdArgs.camera_num; cmdArgs.camera_index ++)
		{
			...
			if(0 != ret)
			{
				fprintf(stderr, "%s: create thread_cssc132[%d] failed\n",__func__,cmdArgs.camera_index);
			}
